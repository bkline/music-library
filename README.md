...
in its entirety. It doesn't take much more than 10 minutes. It is usually
best to avoid doing anything else on the computer where the tests are
running, as the test browser will frequently take over focus as the
active foreground application, so keyboard events you were intending for
some other application will be instead directed to the test browser,
possibly causing a test to fail.
...
pip install openpyl requests selenium
# Show the command-line arguments.
./test-ml.py --help
# Substitute your own values here.
./test-ml.py --b http://localhost:8888 -u tester -p test-password
```
